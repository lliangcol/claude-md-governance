...
tmp="$(mktemp -d)"
cp -R "$root/examples/bad-claude-md/." "$tmp/"
...
if run_gov lint --repo "$tmp" --output "$score_file" --quiet; then
  echo "FAIL bad repo unexpectedly passed"
  exit 1
fi
if run_gov verify --repo "$tmp"; then
  echo "FAIL bad repo verify unexpectedly passed"
  exit 1
fi
score="$("$py" -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["score"])' "$score_file")"
echo "score=$score PASS bad repo demo failed lint and verify as expected: $tmp"
