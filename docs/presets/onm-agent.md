...
对应英文版：[docs/en/presets/onm-agent.md](../en/presets/onm-agent.md)

## 适用场景

`onm-agent` 是面向 onm-agent 类 Java/Maven + Codeup 仓库的示例 preset。它不是通用默认值。
...
- 行为用例路径错误：确认 policy 中 `behavior_tests.case_file` 为 `tests/ai_behavior_cases.onm-agent.json`。

## 默认规则

- CI provider：`codeup`。
- 行为测试用例：`tests/ai_behavior_cases.onm-agent.json`。
